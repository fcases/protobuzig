...
pub const AppConfig = struct {
    ...
    pub const CrossConnectorDef_2 = struct {
        ...
        pub fn skribiAlTeksto(self: *CrossConnectorDef_2, allocator: all.Allocator, t_formato: TekstaFormato) ![]const u8 {
            return try skribiTiponAlTeksto(allocator, CrossConnectorDef_2, @as(*CrossConnectorDef_2, self), t_formato);
        }

        pub fn skribiAlDosiero(self: *CrossConnectorDef_2, allocator: all.Allocator, path: []const u8, t_formato: TekstaFormato) !void {
            try skribiTiponAlDosiero(allocator, CrossConnectorDef_2, @as(*CrossConnectorDef_2, self), path, t_formato);
        }

        pub fn legiElTeksto(allocator: all.Allocator, input: []const u8, t_formato: TekstaFormato) !CrossConnectorDef_2 {
            return try legiTiponElTeksto(allocator, CrossConnectorDef_2, input, t_formato);
        }

        pub fn legiElDosiero(allocator: all.Allocator, path: []const u8, t_formato: TekstaFormato) !CrossConnectorDef_2 {
            return try legiTiponElDosiero(allocator, CrossConnectorDef_2, path, t_formato);
        }

        fn skribiAlProtobufTeksto(self: *const CrossConnectorDef_2, allocator: all.Allocator,ind: []const u8) ![]const u8 {
        ...
        fn legiElProtobufTeksto(allocator: all.Allocator, it: *TokenIterType) !CrossConnectorDef_2 {
        ...
        pub fn seriigiAlBin(self: *CrossConnectorDef_2, allocator: all.Allocator, b_formato: BinaraFormato) ![]const u8 {
            return try seriigiTiponAlBin(allocator, CrossConnectorDef_2, @as(*CrossConnectorDef_2,self), b_formato);
        }

        pub fn seriigiAlDosiero(self: *CrossConnectorDef_2, allocator: all.Allocator, path: []const u8, b_formato: BinaraFormato) !void {
            return try seriigiTiponAlDosiero(allocator, CrossConnectorDef_2, @as(*CrossConnectorDef_2, self), path, b_formato);
        }

        fn seriigi(self: *const CrossConnectorDef_2, buffer: *EncodeBuffer) !usize {
        ...
        pub fn deseriigiElBin(allocator: all.Allocator,input: []const u8, b_formato: BinaraFormato) !CrossConnectorDef_2 {
            return try deseriigiTiponElBin(allocator, CrossConnectorDef_2, input, b_formato);
        }

        pub fn deseriigiElDosiero(allocator: all.Allocator, path: [:0]const u8, b_formato: BinaraFormato) !CrossConnectorDef_2 {
            return try deseriigiTiponElDosiero(allocator, CrossConnectorDef_2, path, b_formato);
        }

        fn deseriigi(allocator: all.Allocator, buffer: *DecodeBuffer, data_length: ?usize) !CrossConnectorDef_2 {
            var mia_Mesagho= try CrossConnectorDef_2.initDefault(allocator);
    ...
    fn legiElProtobufTeksto(allocator: all.Allocator, it: *TokenIterType) !AppConfig {
        ...
        while (it.next()) |tok| {
            ...
            if( equal(u8, tok, "a" ) ) { 
                mia_Mesagho.a =  std.fmt.parseFloat(f64,val) catch 0;
                continue;
            }
            if( equal(u8, tok, "b" ) ) { 
                mia_Mesagho.b =  std.fmt.parseFloat(f64,val) catch 0;
                continue;
            }
            if( equal(u8, tok, "Hola" ) ) { 
                mia_Mesagho.Hola =  std.fmt.parseInt(i32,val,10) catch 0;
                continue;
            }
        }
...
pub const TransportCfg = struct {
    ...
    pub fn initDefault(allocator: all.Allocator) !TransportCfg {
        ...
        self.* = TransportCfg{
            ...
            .ReceiveOwnMsgs = false,
            .Params = "",
            .mnbvX = MCastDefConfig.initDefault(allocator),
        };
        return self.*;
    }

    pub fn skribiAlTeksto(self: *TransportCfg, allocator: all.Allocator, t_formato: TekstaFormato) ![]const u8 {
        return try skribiTiponAlTeksto(allocator, TransportCfg, @as(*TransportCfg, self), t_formato);
    }

    pub fn skribiAlDosiero(self: *TransportCfg, allocator: all.Allocator, path: []const u8, t_formato: TekstaFormato) !void {
        try skribiTiponAlDosiero(allocator, TransportCfg, @as(*TransportCfg, self), path, t_formato);
    }

    pub fn legiElTeksto(allocator: all.Allocator, input: []const u8, t_formato: TekstaFormato) !TransportCfg {
        return try legiTiponElTeksto(allocator, TransportCfg, input, t_formato);
    }

    pub fn legiElDosiero(allocator: all.Allocator, path: []const u8, t_formato: TekstaFormato) !TransportCfg {
        return try legiTiponElDosiero(allocator, TransportCfg, path, t_formato);
    }

    fn skribiAlProtobufTeksto(self: *const TransportCfg, allocator: all.Allocator,ind: []const u8) ![]const u8 {
        var bufro:std.ArrayList(u8)= .empty;
    ...
    fn legiElProtobufTeksto(allocator: all.Allocator, it: *TokenIterType) !TransportCfg {
        ...
        while (it.next()) |tok| {
            ...
            if( equal(u8, tok, "Params" ) ) { 
                mia_Mesagho.Params =  allocator.dupe(u8, val) catch "";
                continue;
            }
            if( equal(u8, tok, "mnbvX" ) ) { 
                const sub_msg = try MCastDefConfig.legiElProtobufTeksto(allocator, it); 
                mia_Mesagho.mnbvX = sub_msg; 
                continue;
            }
        }
    ...
    pub fn seriigiAlBin(self: *TransportCfg, allocator: all.Allocator, b_formato: BinaraFormato) ![]const u8 {
        return try seriigiTiponAlBin(allocator, TransportCfg, @as(*TransportCfg,self), b_formato);
    }

    pub fn seriigiAlDosiero(self: *TransportCfg, allocator: all.Allocator, path: []const u8, b_formato: BinaraFormato) !void {
        return try seriigiTiponAlDosiero(allocator, TransportCfg, @as(*TransportCfg, self), path, b_formato);
    }

    fn seriigi(self: *const TransportCfg, buffer: *EncodeBuffer) !usize {
        var tuta_longo: usize = 0;
 
        const mnbvX_longa = try self.mnbvX.seriigi( buffer );
    ...
    pub fn deseriigiElBin(allocator: all.Allocator,input: []const u8, b_formato: BinaraFormato) !TransportCfg {
        return try deseriigiTiponElBin(allocator, TransportCfg, input, b_formato);
    }

    pub fn deseriigiElDosiero(allocator: all.Allocator, path: [:0]const u8, b_formato: BinaraFormato) !TransportCfg {
        return try deseriigiTiponElDosiero(allocator, TransportCfg, path, b_formato);
    }

    fn deseriigi(allocator: all.Allocator, buffer: *DecodeBuffer, data_length: ?usize) !TransportCfg {
        var mia_Mesagho= try TransportCfg.initDefault(allocator);
        ...
        while (buffer.read_index < end) {
            ...
            if ( field_number == 1 and wire_type == 2 ) 
                ...
                mia_Mesagho.mnbvX = try MCastDefConfig.deseriigi(allocator, buffer, try buffer.decodeVarint() );
        }
...
pub const EndPointDef = struct {
    ...
    fn legiElProtobufTeksto(allocator: all.Allocator, it: *TokenIterType) !EndPointDef {
        ...
        var Nenio_list: std.ArrayList(i32) = .empty;         var Neniam_list: std.ArrayList(f32) = .empty; 
        while (it.next()) |tok| {
            if( equal(u8, tok, "}" ) ) break;
            const val = it.next() orelse return error.InvalidFormat;
            ...
            if( equal(u8, tok, "Nenio" ) ) { 
                try Nenio_list.append(allocator, val); 
                continue;
            }
            if( equal(u8, tok, "Neniam" ) ) { 
                try Neniam_list.append(allocator, val); 
                continue;
            }
        }
